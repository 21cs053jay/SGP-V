...
import React, { useState, useEffect } from "react";
import { useParams, useNavigate } from "react-router-dom";
import axios from "axios"; // Import axios for handling requests
import MyNavbar from "./shared/navbar";
...
const JobApplicationForm = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    data.append("resume", resume); // Append the file
    data.append("jobDetails", JSON.stringify(formData)); // Append form details as JSON

    try {
      // Use axios for the POST request with multipart/form-data
      const response = await axios.post("http://localhost:5000/api/submitApplication", data, {
        headers: {
          "Content-Type": "multipart/form-data", // Let axios automatically handle this
        },
      });
